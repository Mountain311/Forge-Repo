...
# --- 1. COMPILE YAML INTO PURE PYTHON ---
...
if "recovery_agent" not in agent_registry:
    print("❌ CRITICAL: recovery_agent was not found. Aborting.")
    exit(1)
...
if os.path.exists(package_json_path):
    ...
    try:
        ...
        print(f"✅ Updated package.json default")
    except KeyError:
        print("⚠️ Could not find forge.endpointId in package.json")
...
if os.path.exists(vscode_settings_dir):
    ...
    if "forge.endpointId" in settings_data:
        ...
        print(f"✅ Cleared workspace override to refresh GUI.")

# --- 4. UPDATE LOCAL TXT FILE (Restored for testing scripts) ---
endpoint_file_path = os.path.join(backend_dir, "endpoint_id.txt")
with open(endpoint_file_path, "w", encoding='utf-8') as f:
    f.write(endpoint_id)
print(f"✅ Updated local backend file: {endpoint_file_path}")